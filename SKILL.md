...
3. **Draft as DocxXML.** XML, not Markdown — callouts, grids, colored text, and checkboxes don't exist in Markdown and silently degrade if you draft in it. **Diagram rule:** flowcharts, sequence diagrams, architecture diagrams, state machines, and hierarchy diagrams should default to editable Feishu whiteboards (`<whiteboard type="mermaid">`), not screenshots, Mermaid code fences, or plain images. Use images only for non-diagram visuals or cases a whiteboard cannot express. Apply the principles below; for block syntax read `references/feishu-docxxml.md`, and consult `lark-doc`'s component reference to pick blocks (`lark-cli skills read lark-doc references/style/lark-doc-style.md`) — but structure, color budget, and richness are governed by this skill's principles, not deferred to it.
...
- [ ] **Diagrams render** — every whiteboard exported (`docs +media-download`) and eyeballed, not just present in the XML; broken ones fixed via `docs +whiteboard-update`.
