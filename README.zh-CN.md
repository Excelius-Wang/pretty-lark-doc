...
完整填好的范例（非骨架）见 `assets/examples/module-design.md` —— 一篇真实的模块设计文档，含三张已渲染的白板。
...
| `assets/examples/` | 完整范例文档（已填好，供参照） |
| `README.md` | 英文 README |
...
创建需 **user 身份**且 scope 含 `docx:document:create`，且应用必须在飞书开放平台「启用」（否则 user / bot 都报 `20069`）。用 `lark-cli auth status --json` 确认 `user.status` = `ready`。
